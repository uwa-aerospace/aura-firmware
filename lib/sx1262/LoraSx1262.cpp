...
bool LoraSx1262::begin() {
  ...
  SPI.begin(RADIO_CLK, RADIO_MISO, RADIO_MOSI);

  //Set I/O pins based on the configuration
  //See .h file for pinout diagram
  digitalWrite(RADIO_NSS, 1);  //High = inactive
  pinMode(RADIO_NSS,OUTPUT);
  digitalWrite(RADIO_NSS, 1);  //High = inactive

  if (RADIO_RESET != -1) {
    digitalWrite(RADIO_RESET, 1);  //High = inactive
    pinMode(RADIO_RESET,OUTPUT);
  }
  
  pinMode(RADIO_DIO1, INPUT);  //Radio interrupt pin.  Goes high when we receive a packet

  if (RADIO_RESET != - 1) {
    //Hardware reset the radio by toggling the reset pin
    digitalWrite(RADIO_RESET, 0); delay(50);
...
bool LoraSx1262::sanityCheck() {
  ...
  digitalWrite(RADIO_NSS, 0);  //CS Low = Enabled
  ...
  digitalWrite(RADIO_NSS, 1);  //CS High = Disabled
...
void LoraSx1262::configureRadioEssentials() {
  ...
  delay(50);//Give time for the radio to proces command
  ...
  SPI.transfer(spiBuff,2);
  digitalWrite(RADIO_NSS,1); //Disable radio chip-select
  delay(50);                 //Give time for radio to process the command
  ...
  digitalWrite(RADIO_NSS,0);
  spiBuff[0] = 0x0D;    // Opcode for "WriteRegister"
  spiBuff[1] = 0x08;    // High byte of register address (0x08AC)
  spiBuff[2] = 0xAC;    // Low byte of register address (0x08AC)
  spiBuff[3] = 0x96;    // RX boosted mode 0x96, power saving mode 0x94
  SPI.transfer(spiBuff, 4);
  digitalWrite(RADIO_NSS,1);
  delay(50);
...
void LoraSx1262::transmit(char *data, int dataLen) {
...
*/
int LoraSx1262::lora_receive_async(char* buff, int buffMaxLen) {
...
Returns payload size (1-255) when a packet with a non-zero payload is received. If packet received is larger than the buffer provided, this will return buffMaxLen
*/
int LoraSx1262::lora_receive_blocking(char *buff, int buffMaxLen, uint32_t timeout) {
  setModeReceive(); //Sets the mode to receive (if not already in receive mode)
  ...
  while (digitalRead(RADIO_DIO1) == false) {
