...
class LoraSx1262 {
  public:
    ...
    void transmit(char* data, int dataLen);
    int lora_receive_async(char* buff, int buffMaxLen); /*Checks to see if a lora packet was received yet, returns the packet if available*/
    int lora_receive_blocking(char* buff, int buffMaxLen, uint32_t timeout); /*Waits until a packet is received, with an optional timeout*/
